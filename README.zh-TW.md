...
> ### 📘 [先看它做出什麼，再決定要不要往下讀 →](https://claude.ai/code/artifact/c783cc69-99a2-4bff-ada9-c10823134e8b)
>
> 一頁寫給**不寫程式的人** —— 創作者、辦活動的人、手機裡塞滿片段的人。
...
| 閘門 | 擋什麼 | 它記下的那次事故 |
|---|---|---|
| Decisions | 沒有 `decisions.json`；屬於使用者的選擇（響度、字幕、end card）被靜默預設，或偏離預設卻沒寫下 why | 連續兩支片在「該問而未問」的情況下自行做了響度正規化，兩次都回報「完成」 |
| Audio | 死寂 >0.8s、結尾靜音（<-32 dBFS）、爆音 | 收尾卡沒有聲音，發生過兩次 |
| MusicBed | 音樂床比影片先死（用無音樂版相減量測），或根本沒有成對版本可以相減 | 從舊剪繼承來的尾端淡出壓在最後一句話上；最後 2.5 秒因為還有人在講話而通過了 gate_audio |
| Deliverables | 專案根目錄缺音樂版／無音樂版成對檔案或 `cover.jpg` | 只交一半的成品 ——「兩個版本都要出」寫成文字從來擋不住這件事 |
| CoverColour | 封面副標不是 house 金色，直接量測渲染後的像素 | 呼叫端直接傳了一個顏色字面值，而沒有任何地方真的去看那張圖 |
| Cover | 沒封面、第一幀不是封面、標題小到 feed 裡看不清、副標寬度沒跟著標題 | 整支片沒有封面；副標浮在一個和主標無關的寬度上；或是用橫幅素材做出來的封面 —— 尺寸對，但畫面是躺著的 |
| Captions | 超出安全區、未宣告的樣式、字幕偏離宣告的錨點、巢狀顏色標籤 | 所有字幕都疊在 50% 的位置；文字超出安全區；`LiteRT` 在 `LiteRT-LM.js` 裡被重複上色 |
| Typography | 字體或字級錯、黑色描邊而非 house 陰影、house 硬切卻出現 `\fad`、整片全白沒有金色關鍵字（僅檢查用 `Speech` 樣式的字幕）、翻一半的雙語字幕 | 一次重建用了 62pt、6px 黑色描邊，每一行都加淡入淡出；還有一次整批字幕全部算成純白（2026-08-17） |
| Structure | 第一秒內沒有 hook、沒有 end card、影片沒有結束在 end card 上，或 end card 沒有宣告任何文字 | 因為沒有任何東西要求，hook 和收尾卡每次都從頭重想；或是一張沒有宣告任何文字的收尾卡 —— 那其實是最後一個片段的截圖，卻通過了其他所有檢查 |
| Sync | 字幕的字不在它底下的音軌裡：首字被切、晚超過 1 秒、錯行壓錯鏡頭、`words.json` 過期 | 用手工比對表格才找出來的四個對位錯誤 |
| Pill | 整個缺失、方角（ASS 方框而非 PIL 膠囊）、貼滿邊、偏離 18% 位置、有淡入 | pill 被畫成 ASS 的粗糙方框；而這道閘門本身在 pill 根本不存在時回報 PASS |
...
| AudioPolicy | 有段落的音軌沒有人做過決定、留或靜音卻沒寫下 why，或是寫了 policy 卻沒照著算圖——靜音段與保留段的落差直接在無音樂版上量測 | 一支 48 秒、沒有人說話的美食花絮，整段都讓現場環境音跑著就出貨了 —— 因為「環境音就是配樂」被套用到每一顆鏡頭，而不是只套用在聲音本身就是主角的那幾顆。每道閘門都是綠的，使用者一次就聽出來 |
| Delivery | PTS≠0 的黑首幀、音視訊長度不符 | concat 產生的第一格黑畫面 |
