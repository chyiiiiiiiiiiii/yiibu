...
def scan(src_dir, work_dir, model="large-v3", skip=(), quiet_db=-25.0):
    ...
    loud, out = [], {}
    for f in files:
        db = _max_volume_db(os.path.join(src_dir, f))
    ...
    if loud:
        listp = os.path.join(work_dir, "_asr_list.json")
        rawp = os.path.join(work_dir, "_asr_raw.json")
        json.dump([os.path.join(src_dir, f) for f in loud], open(listp, "w"))
        r = subprocess.run([_venv_python(), "-c", _WORKER, listp, model, rawp],
                           text=True)
        if r.returncode or not os.path.exists(rawp):
            raise SystemExit("ASR worker failed — is faster-whisper installed "
                             "in .venv? (python3 doctor.py)")
        raw = json.load(open(rawp, encoding="utf-8"))
        for f in loud:
            rows = raw.get(os.path.join(src_dir, f), [])
            ...
            if speech:
                ...
                row["heard"] = text[:120]
            out[f] = row
            mark = "SPEECH" if speech else "  --  "
            print(f"  {f:44s} {mark}  nsp {nsp:.2f}"
                  + (f"  {len(words)} words" if speech else f"  ({text[:36]})"))
        for p in (listp, rawp):
...
def main():
    ...
    a = ap.parse_args()
    out = scan(a.source_dir, a.work_dir, a.model, set(a.skip))
    if a.json:
